<div>
    <div class="mb-6 flex items-center justify-between gap-4">
        ...
        @if ($editingId === null)
            <button wire:click="create" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500">
                New category
            </button>
        @endif
    </div>
    ...
    @if ($editingId !== null)
        <form wire:submit="save" class="mb-6 space-y-4 rounded-lg border border-gray-200 p-6 dark:border-gray-800">
            ...
            <div>
                ...
                <textarea id="description" wire:model="description" rows="3" class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-800"></textarea>
            </div>
                ...
                @forelse ($categories as $category)
                    <tr wire:key="admin-category-{{ $category->id }}">
                        ...
                        <td class="px-4 py-3 text-right">
                            <button wire:click="edit({{ $category->id }})" class="mr-3 text-indigo-600 hover:underline dark:text-indigo-400">Edit</button>
                            <button
