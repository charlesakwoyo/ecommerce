<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ $order->user?->name ?? 'Guest' }} &middot; placed {{ $order->created_at->format('M j, Y \a\t g:ia') }}
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div class="space-y-6">
            <div>
                <h2 class="mb-2 text-sm font-semibold uppercase text-gray-500 dark:text-gray-400">Update status</h2>
                <div class="flex items-center gap-3 rounded-lg border border-gray-200 p-4 dark:border-gray-800">
                    <select wire:model="status" class="flex-1 rounded-md border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800">
                        @foreach ($statuses as $case)
                            <option value="{{ $case->value }}">{{ $case->label() }}</option>
                        @endforeach
                    </select>
                    <x-button wire:click="updateStatus" type="button">Update</x-button>
                </div>
            </div>
